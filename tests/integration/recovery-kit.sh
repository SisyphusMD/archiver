...
archiver backup || die "backup (no kit) exited non-zero"
...
archiver backup || die "backup (kit) exited non-zero"
...
archiver backup || die "backup (unchanged) exited non-zero"
...
archiver backup || die "backup (changed) exited non-zero"
...
log "maintenance with forced exhaustive prune must leave the kit files alone"
echo "more content" >>"$FIXTURES/file.txt"
archiver backup || die "backup exited non-zero"
archiver maintenance exhaustive || die "maintenance exhaustive exited non-zero"
[ -f "$KIT" ] || die "exhaustive prune removed the recovery kit"
[ -f "$README" ] || die "exhaustive prune removed the kit README"
