...
docker run -it --rm \
  ...
  ghcr.io/sisyphusmd/archiver:0.6.2 setup
```
...
2. Clone repository:
   ```bash
   git clone --branch v0.6.2 https://github.com/SisyphusMD/archiver.git
   cd archiver
   ```
...
services:
  archiver:
    image: ghcr.io/sisyphusmd/archiver:0.6.2
...
docker run --rm -it \
  ...
  ghcr.io/sisyphusmd/archiver:0.6.2 \
  archiver restore
```
