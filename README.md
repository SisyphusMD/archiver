...
docker run -it --rm \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0 init
```
...
- `forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0` - exact version (recommended; this line always names the current release)
...
docker run --rm \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0 run backup
```
...
docker run -d --name archiver-restore \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0
...
docker run --rm \
  ...
  -v /path/to/secrets:/run/secrets:ro \
  -e SNAPSHOT_ID=myservice \
  forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0 run snapshot-exists
...
docker run --rm \
  ...
  -e OVERWRITE=1 \
  -v /path/to/restore/destination:/mnt/restore \
  forgejo.bryantserver.com/sisyphusmd/archiver:0.10.0 run auto-restore
```
