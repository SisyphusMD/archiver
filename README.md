...
docker run -it --rm \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1 init
```
...
services:
  ...
  archiver:
    ...
    image: forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1
...
- `forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1` - exact version (recommended; this line always names the current release)
...
docker run --rm \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1 run backup
```
...
docker run -d --name archiver-restore \
  ...
  forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1
...
docker run --rm \
  ...
  -v /path/to/secrets:/run/secrets:ro \
  -e SNAPSHOT_ID=myservice \
  forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1 run snapshot-exists
...
docker run --rm \
  ...
  -e OVERWRITE=1 \
  -v /path/to/restore/destination:/mnt/restore \
  forgejo.bryantserver.com/sisyphusmd/archiver:0.9.1 run auto-restore
```
